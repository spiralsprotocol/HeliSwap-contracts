...
describe("HeliSwap Tests", function () {
  ...
  describe("", async () => {
    ...
    beforeEach(async () => {
      ...
      // @ts-ignore
      tokenA = await hardhat.hethers.getContractAt(
        ERC20,
        newTokenA.tokenAddress
      );
    });
    ...
    describe("Adding HTS liquidity", function () {
      it("should be able to add HTS/HTS liquidity", async () => {
        ...
        // @ts-ignore
        const tokenB = await hardhat.hethers.getContractAt(
          ERC20,
          newTokenB.tokenAddress
        );
      ...
      it("should be able to add HTS/ERC20 liquidity", async () => {
        ...
        const erc20Address = await deployMintERC20(
          deployer.address,
          amountERC20,
          ERC20_NAME,
          ERC20_SYMBOL
  ...
  describe("", async () => {
    ...
    describe("Adding ERC20 liquidity", async () => {
      it("should be able to add ERC20/ERC20 liquidity", async () => {
        ...
        const otherERC20Address = await deployMintERC20(
          deployer.address,
          ERC20_SUPPLY,
          otherERC20Name,
          otherERC20Symbol
        ...
        // @ts-ignore
        const otherERC20 = await hardhat.hethers.getContractAt(
          ERC20,
          otherERC20Address
        );
    ...
    describe("ERC20 related swaps", async () => {
      ...
      it("should be able to swap ERC20/HBAR", async () => {
        ...
        await executeSwapHBAR(erc20, amountIn, amountOut, deployer.address);
      });
    ...
    describe("ERC20 related removals", async () => {
      ...
      it("should be able to remove ERC20/HBAR liquidity", async () => {
        ...
        // @ts-ignore
        const pairContractAddress = Utils.getCreate2Address(factory.address, [
          erc20.address,
          whbar.address,
        ]);
        // @ts-ignore
        const pairContract = await hardhat.hethers.getContractAt(
          PAIR,
          pairContractAddress
        );
        ...
        await executeRemoveLiquidityHBAR(
          erc20,
          removableLPAmount,
          amountAOut,
          amountBOut,
  ...
  async function executeAddLiquidityHBAR(
    ...
    const pairAddress = Utils.getCreate2Address(factory.address, [
      whbar.address,
      token0.address,
    ]);
  ...
  async function executeSwap(
    ...
    // @ts-ignore
    let pairContract = await hardhat.hethers.getContractAt(
      "UniswapV2Pair",
      pairAddress
    );

    const reservesBeforeSwap = await router.getReserves(
      token0.address,
      token1.address
    );
  ...
  async function executeRemoveLiquidity(
    ...
    const reservesBeforeRemoveLP = await router.getReserves(
      token0.address,
      token1.address
    );
    const totalSupplyBeforeRemoveLP = BigNumber.from(
